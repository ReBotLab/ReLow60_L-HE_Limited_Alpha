...
static const tusb_desc_device_t desc_device = {
    ...
    // IAD composite device class (required for CDC + HID)
...
#define CONFIG_TOTAL_LEN                                                       \
  (TUD_CONFIG_DESC_LEN + 2 * TUD_HID_DESC_LEN + TUD_HID_INOUT_DESC_LEN +      \
   TUD_CDC_DESC_LEN + XINPUT_DESC_LEN)
...
static void generate_desc_configuration(uint8_t *dst) {
  ...
  const uint8_t src[] = {
      ...
      TUD_HID_INOUT_DESCRIPTOR(USB_ITF_RAW_HID, 0, HID_ITF_PROTOCOL_NONE,
                               ...
                               polling_interval),
      // CDC interface descriptor (virtual serial port for measurement output)
      TUD_CDC_DESCRIPTOR(USB_ITF_CDC, 0, EP_IN_ADDR_CDC_NOTIF, 8,
                         EP_OUT_ADDR_CDC_DATA, EP_IN_ADDR_CDC_DATA,
                         CDC_DATA_EP_SIZE),

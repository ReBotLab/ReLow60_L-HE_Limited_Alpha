...
bool board_factory_reset_requested(void) {
  ...
  gpio_struct.gpio_pins = GPIO_PINS_11;
  gpio_struct.gpio_pull = GPIO_PULL_DOWN;
  gpio_init(GPIOF, &gpio_struct);

  // Poll BOOT0 pin (PF11) for 500ms to give the user time to press it
  uint32_t start = timer_read();
  while (timer_elapsed(start) < 500) {
    if (gpio_input_data_bit_read(GPIOF, GPIO_PINS_11) == SET)
      return true;
  }
  return false;
}

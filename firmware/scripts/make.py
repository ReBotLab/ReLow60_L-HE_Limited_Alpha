...
if kb_json.analog.invert_adc_keys is not None:
    bitmap = 0
    for key in kb_json.analog.invert_adc_keys:
        bitmap |= 1 << (key - 1)
    build_flags.define("MATRIX_INVERT_ADC_KEYS_BITMAP", hex(bitmap))

if kb_json.analog.fixed_onoff_keys is not None:
    build_flags.define(
        "MATRIX_FIXED_ONOFF_KEYS",
        utils.to_c_array([key - 1 for key in kb_json.analog.fixed_onoff_keys]),
    )
    build_flags.define(
        "MATRIX_FIXED_ONOFF_ON_DELTA", kb_json.analog.fixed_onoff_on_delta
    )
    build_flags.define(
        "MATRIX_FIXED_ONOFF_OFF_DELTA", kb_json.analog.fixed_onoff_off_delta
    )
